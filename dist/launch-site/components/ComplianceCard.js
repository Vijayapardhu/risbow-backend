...
exports.ComplianceCard = ComplianceCard;
function ComplianceCard({ id, title, subtitle, bullets, icon }) {
    return (<section id={id} className="group relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-8 shadow-sm shadow-brand-accent/10 transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <ul className="mt-6 space-y-3 text-sm leading-relaxed text-slate-600">
        {bullets.map((item) => (<li key={item} className="flex items-start gap-3">
...
//# sourceMappingURL=ComplianceCard.js.map